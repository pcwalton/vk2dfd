// vk2dfd/etc/genrust.c
//
// Place this in `dfdutils/genrust.c`, compile it, and run.
...
int main() {
    printf("// Generated by `genrust`. Don't edit.\n\n");
    printf("use phf::{phf_map, Map};\n\n");
    printf("pub(crate) static VK_TO_DFD: Map<u32, &[u32]> = phf_map! {\n");

    for (int i = 0; i < sizeof(ALL_VK_FORMATS) / sizeof(ALL_VK_FORMATS[0]); i++) {
        VkFormat format = ALL_VK_FORMATS[i];
        uint32_t *dfd = vk2dfd(format);
        if (dfd == NULL)
            continue;

        printf("    %uu32 => &[", format);
        uint32_t dfdSize = dfd[0] / sizeof(uint32_t);
        for (int j = 0; j < dfdSize; j++)
            printf("%s0x%08x", j == 0 ? "" : ", ", dfd[j]);
        free(dfd);
        printf("],\n");
    }
